...
function write_buffer_string(pos, len, value, using_key, item_id) {
  for (let a = 0; a < len; a++) {
    if (using_key)
      encoded_buffer_file[pos + a] =
        value.charCodeAt(a) ^
        items_secret_key.charCodeAt((a + item_id) % items_secret_key.length);
  ...
  if (using_key)
    for (let a = 0; a < len; a++)
      result += String.fromCharCode(
        buffer[a + pos] ^
          items_secret_key.charCodeAt((item_id + a) % items_secret_key.length),
      );
  else
    for (let a = 0; a < len; a++)
...
document
  .getElementById("decode_items_dat")
...
document
  .getElementById("decode_items_dat_editor")
  .addEventListener("click", function () {
    var input = document.createElement("input");
    input.type = "file";
    input.style.display = "none";
...
document
  .getElementById("encode_items_dat")
  .addEventListener("click", function () {
    var input = document.createElement("input");
    input.type = "file";
    input.style.display = "none";
  ...
  if (using_txt) {
    ...
    for (let a = 0; a < result.length; a++) {
      ...
      if (result1[0] == "version") {
        ...
        write_buffer_number(2, 4, Number(result1[1]));
        ...
        if (result1[16].includes("r"))
          encoded_buffer_file[mem_pos++] = Number(result1[16].slice(0, -1));
        ...
        if (version >= 23) {
          write_buffer_number(mem_pos, 2, result1[53]);
          mem_pos += 2;
          write_buffer_number(mem_pos, 2, result1[54]);
          mem_pos += 2;
        }
        if (version >= 24) {
          encoded_buffer_file[mem_pos++] = Number(result1[55]);
        }
        if (version >= 25) {
          var hit_sound_fx = result1[56] || "";
          write_buffer_number(mem_pos, 2, hit_sound_fx.length);
          mem_pos += 2;
          write_buffer_string(mem_pos, hit_sound_fx.length, hit_sound_fx);
          mem_pos += hit_sound_fx.length;
          write_buffer_number(mem_pos, 4, result1[57]);
          mem_pos += 4;
        }
        if (version >= 26) {
          encoded_buffer_file[mem_pos++] = Number(result1[58]);
        }
      }
    }
  } else {
    ...
    for (let a = 0; a < result.item_count; a++) {
      ...
      if (check_last_char(result.items[a].break_hits.toString(), "r"))
        encoded_buffer_file[mem_pos++] = Number(
          result.items[a].break_hits.toString().slice(0, -1),
        );
      else
        encoded_buffer_file[mem_pos++] = Number(result.items[a].break_hits) * 6;
      ...
      if (result.version >= 22) {
        ...
        write_buffer_string(
          mem_pos,
          result.items[a].str_version_22.length,
          result.items[a].str_version_22,
        );
        mem_pos += result.items[a].str_version_22.length;
      }
      if (result.version >= 23) {
        write_buffer_number(mem_pos, 2, result.items[a].seed_1);
        mem_pos += 2;
        write_buffer_number(mem_pos, 2, result.items[a].seed_2);
        mem_pos += 2;
      }
      if (result.version >= 24) {
        encoded_buffer_file[mem_pos++] = Number(result.items[a].int_version_24);
      }
      if (result.version >= 25) {
        var hit_sound_fx = result.items[a].hit_sound_fx || "";
        write_buffer_number(mem_pos, 2, hit_sound_fx.length);
        mem_pos += 2;
        write_buffer_string(mem_pos, hit_sound_fx.length, hit_sound_fx);
        mem_pos += hit_sound_fx.length;
        write_buffer_number(mem_pos, 4, result.items[a].hit_sound_fx_hash);
        mem_pos += 4;
      }
      if (result.version >= 26) {
        encoded_buffer_file[mem_pos++] = Number(result.items[a].int_version_26);
      }
    }
  }
}
...
function item_decoder(file, using_editor) {
  ...
  reader.onload = function (e) {
    ...
    for (let a = 0; a < item_count; a++) {
      ...
      var data_position_80 = hex(
        arrayBuffer.slice(mem_pos, mem_pos + 80),
        document.getElementById("using_txt_mode").checked,
      ).toUpperCase();
      mem_pos += 80;
      ...
      if (version >= 23) {
        var seed_1 = read_buffer_number(arrayBuffer, mem_pos, 2);
        mem_pos += 2;
        var seed_2 = read_buffer_number(arrayBuffer, mem_pos, 2);
        mem_pos += 2;
      }

      if (version >= 24) {
        var int_version_24 = arrayBuffer[mem_pos++];
      }

      if (version >= 25) {
        len = read_buffer_number(arrayBuffer, mem_pos, 2);
        mem_pos += 2;
        var hit_sound_fx = read_buffer_string(arrayBuffer, mem_pos, len);
        mem_pos += len;

        var hit_sound_fx_hash = read_buffer_number(arrayBuffer, mem_pos, 4);
        mem_pos += 4;
      }

      if (version >= 26) {
        var int_version_26 = arrayBuffer[mem_pos++];
      }
      ...
      if (document.getElementById("using_txt_mode").checked) {
        data_json.items[a].seed_color =
          `${seed_color_a},${seed_color_r},${seed_color_g},${seed_color_b}`;
        data_json.items[a].seed_overlay_color =
          `${seed_overlay_color_a},${seed_overlay_color_r},${seed_overlay_color_g},${seed_overlay_color_b}`;
      } else {
        data_json.items[a].seed_color = {};
      ...
      data_json.items[a].seed_1 = seed_1;
      data_json.items[a].seed_2 = seed_2;
      data_json.items[a].int_version_24 = int_version_24;
      data_json.items[a].hit_sound_fx = hit_sound_fx;
      data_json.items[a].hit_sound_fx_hash = hit_sound_fx_hash;
    ...
    if (using_editor) {
      if (!$.fn.dataTable.isDataTable("#itemsList")) {
        ...
        document
          .getElementById("save_items_dat_div")
        ...
        $("#itemsList")
          ...
          .buttons()
          .container()
          .appendTo("#itemsList_wrapper .col-md-6:eq(0)");
      ...
      for (let a = 0; a < item_count; a++) {
        ...
        result[a][2] =
          `<center><button class="btn btn-primary" onclick="editItems(${a})">Edit/Info</button></center>`;
      }
      ...
      if (document.getElementById("using_txt_mode").checked) {
        ...
        for (let a = 0; a < item_count; a++)
          to_txt_result +=
            "add_item\\" + Object.values(data_json.items[a]).join("\\") + "\n";
...
function editItems(posArray) {
  ...
  document.getElementById("editable_type").value =
    data_json.items[posArray].editable_type;
  document.getElementById("item_category").value =
    data_json.items[posArray].item_category;
  document.getElementById("action_type").value =
    data_json.items[posArray].action_type;
  document.getElementById("hit_sound_type").value =
    data_json.items[posArray].hit_sound_type;
  ...
  document.getElementById("texture_hash").value =
    data_json.items[posArray].texture_hash;
  document.getElementById("item_kind").value =
    data_json.items[posArray].item_kind;
  document.getElementById("val1").value = data_json.items[posArray].val1;
  document.getElementById("texture_x").value =
    data_json.items[posArray].texture_x;
  document.getElementById("texture_y").value =
    data_json.items[posArray].texture_y;
  document.getElementById("spread_type").value =
    data_json.items[posArray].spread_type;
  document.getElementById("is_stripey_wallpaper").value =
    data_json.items[posArray].is_stripey_wallpaper;
  document.getElementById("collision_type").value =
    data_json.items[posArray].collision_type;
  document.getElementById("break_hits").value =
    data_json.items[posArray].break_hits;
  document.getElementById("drop_chance").value =
    data_json.items[posArray].drop_chance;
  document.getElementById("clothing_type").value =
    data_json.items[posArray].clothing_type;
  document.getElementById("rarity").value = data_json.items[posArray].rarity;
  document.getElementById("max_amount").value =
    data_json.items[posArray].max_amount;
  document.getElementById("extra_file").value =
    data_json.items[posArray].extra_file;
  document.getElementById("extra_file_hash").value =
    data_json.items[posArray].extra_file_hash;
  document.getElementById("audio_volume").value =
    data_json.items[posArray].audio_volume;
  document.getElementById("pet_name").value =
    data_json.items[posArray].pet_name;
  document.getElementById("pet_prefix").value =
    data_json.items[posArray].pet_prefix;
  document.getElementById("pet_suffix").value =
    data_json.items[posArray].pet_suffix;
  document.getElementById("pet_ability").value =
    data_json.items[posArray].pet_ability;
  document.getElementById("seed_base").value =
    data_json.items[posArray].seed_base;
  document.getElementById("seed_overlay").value =
    data_json.items[posArray].seed_overlay;
  document.getElementById("tree_base").value =
    data_json.items[posArray].tree_base;
  document.getElementById("tree_leaves").value =
    data_json.items[posArray].tree_leaves;
  document.getElementById("seed_color").value = Object.values(
    data_json.items[posArray].seed_color,
  ).toString();
  document.getElementById("seed_overlay_color").value = Object.values(
    data_json.items[posArray].seed_overlay_color,
  ).toString();
  document.getElementById("grow_time").value =
    data_json.items[posArray].grow_time;
  document.getElementById("val2").value = data_json.items[posArray].val2;
  document.getElementById("is_rayman").value =
    data_json.items[posArray].is_rayman;
  document.getElementById("extra_options").value =
    data_json.items[posArray].extra_options;
  document.getElementById("texture2").value =
    data_json.items[posArray].texture2;
  document.getElementById("extra_options2").value =
    data_json.items[posArray].extra_options2;
  document.getElementById("pos_80_data").value =
    data_json.items[posArray].data_position_80;
  document.getElementById("punch_options").value =
    data_json.items[posArray].punch_options;
  document.getElementById("data_version_12").value =
    data_json.items[posArray].data_version_12;
  document.getElementById("int_version_13").value =
    data_json.items[posArray].int_version_13;
  document.getElementById("int_version_14").value =
    data_json.items[posArray].int_version_14;
  document.getElementById("data_version_15").value =
    data_json.items[posArray].data_version_15;
  document.getElementById("str_version_15").value =
    data_json.items[posArray].str_version_15;
  document.getElementById("str_version_16").value =
    data_json.items[posArray].str_version_16;
  document.getElementById("int_version_17").value =
    data_json.items[posArray].int_version_17;
  document.getElementById("int_version_18").value =
    data_json.items[posArray].int_version_18;
  document.getElementById("int_version_19").value =
    data_json.items[posArray].int_version_19;
  document.getElementById("int_version_21").value =
    data_json.items[posArray].int_version_21;
  document.getElementById("str_version_22").value =
    data_json.items[posArray].str_version_22;
  document
    .getElementById("editItemsButton")
    .setAttribute("onclick", `processEditItems(${posArray})`);
}
...
function processEditItems(posArray) {
  ...
  data_json.items[posArray].spread_type =
    document.getElementById("spread_type").value;
  data_json.items[posArray].is_stripey_wallpaper = document.getElementById(
